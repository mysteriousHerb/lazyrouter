...
_TOOL_CALL_MODEL_CACHE_PER_SESSION_MAX = 50
_TOOL_CALL_MODEL_CACHE_LOCK = threading.Lock()
...
def tool_cache_set(
    ...
    with _TOOL_CALL_MODEL_CACHE_LOCK:
        ...
        # Enforce per-session limit to prevent flooding from a single session.
        prefix = f"{session_key}::"
        session_keys = [k for k in _TOOL_CALL_MODEL_CACHE if k.startswith(prefix)]
        while len(session_keys) > _TOOL_CALL_MODEL_CACHE_PER_SESSION_MAX:
            oldest_key = session_keys.pop(0)
            _TOOL_CALL_MODEL_CACHE.pop(oldest_key, None)
