...
def _assemble_non_streaming_response(
    ctx: Any, response: Dict[str, Any], show_model_prefix: bool
) -> Dict[str, Any]:
    """Assemble the final non-streaming response dict, cache tool calls, inject lazyrouter metadata."""
...
async def _logged_stream(
    ...
    async def _replace_stream(retry_extra_kwargs: Dict[str, Any]) -> None:
        ...
        current_response = await router.chat_completion(
            ...
            _lazyrouter_input_request=ctx.request.model_dump(exclude_none=True),
            **retry_extra_kwargs,
            **ctx.provider_kwargs,
        )
                    ...
                    try:
                        ...
                        for choice in chunk_data.get("choices", []):
                            ...
                            for tool_call in (delta.get("tool_calls", []) or []):
                                ...
                                if tc_name:
                                    streamed_tool_names.add(tc_name)
                                if tc_id or tc_name:
                                    streamed_tool_calls.append(
                                        {"id": tc_id or "", "name": tc_name or ""}
                                    )
