...
def print_analysis(entries: List[Dict[str, Any]], savings: Dict[str, Any]):
    ...
    print("-" * 80)
    print(f"{'Req':>3} {'Msgs':>5} {'System':>10} {'Tools':>10} {'History':>10} {'Total':>12} {'Latency':>10}")
    print("-" * 80)
    ...
    # Cost estimation (Anthropic Claude 3.5 Sonnet pricing)
    input_price_per_1k = 0.003  # $3 per 1M tokens
    cache_write_price_per_1k = 0.00375  # $3.75 per 1M tokens
    cache_read_price_per_1k = 0.0003  # $0.30 per 1M tokens
    ...
    cost_with = (tokens_with_cache_write / 1000) * cache_write_price_per_1k + \
                (tokens_with_cache_read / 1000) * input_price_per_1k + \
                ((savings['avg_static_size'] * (savings['request_count'] - 1)) / 4 / 1000) * cache_read_price_per_1k
    ...
    print(f"  Savings:         ${cost_without - cost_with:.4f} ({(1 - cost_with/cost_without)*100:.1f}%)")
